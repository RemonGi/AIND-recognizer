...
class SelectorCV(ModelSelector):
    ...
    def select(self):
        ...
        if len(self.sequences) < 2:
            print("NO PASA NADA. Secuencias = ".format(self.sequences))
        ...
        for components in range(self.min_n_components, self.max_n_components + 1):
            try:
                split_method = KFold(min(3, len(self.sequences))).split(self.sequences)
                score = 0
                for train_idx, test_idx in split_method:
                    train_X, train_lengths = combine_sequences(train_idx, self.sequences)
                    try:
                        hmm_model = GaussianHMM(n_components=components, covariance_type="diag", n_iter=1000, random_state=self.random_state, verbose=False).fit(train_X, train_lengths)
                ...
                if score > max_score:
                    max_score = score
                    b_model = self.base_model(components)
